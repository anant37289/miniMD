...
void test(x_view_type x_local){
  CkPrintf("[DEBUG] Neighbor::test: Starting...\n");
  
  // MINIMAL TEST - just 2 captures
  // auto x_local = atom.x;
  // auto nlocal_local = nlocal;

  CkPrintf("[DEBUG] Neighbor::test: nlocal=%d. Synchronizing device...\n", 10);
  cudaError_t err = cudaDeviceSynchronize();
  if (err != cudaSuccess) CkPrintf("[ERROR] Pre-test sync failed: %s\n", cudaGetErrorString(err));

  CkPrintf("[DEBUG] Neighbor::test: Launching minimal_test kernel...\n");
  Kokkos::parallel_for("minimal_test", Kokkos::RangePolicy<Kokkos::Cuda>(0, 10), KOKKOS_LAMBDA(int i){
    // Just access x to make sure kernel does something
    volatile double sum = x_local(i,0) + x_local(i,1) + x_local(i,2);
    (void)sum; // suppress unused warning
  });
  
  CkPrintf("[DEBUG] Neighbor::test: Kernel launched. Synchronizing...\n");
  Kokkos::fence();
  err = cudaDeviceSynchronize();
  if (err != cudaSuccess) CkPrintf("[ERROR] Post-test sync failed: %s\n", cudaGetErrorString(err));
  
  CkPrintf("[DEBUG] minimal_test completed successfully!\n");
}
