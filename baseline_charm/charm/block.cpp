...
void Block::init() {
  ...
  compute_instance = Kokkos::DefaultExecutionSpace(compute_stream);
  h2d_instance = Kokkos::DefaultExecutionSpace(h2d_stream);
  d2h_instance = Kokkos::DefaultExecutionSpace(d2h_stream);
  pack_instance = Kokkos::DefaultExecutionSpace(pack_stream);
  unpack_instance = Kokkos::DefaultExecutionSpace(unpack_stream);
...
void Block::printConfig() {
  if (thisIndex == 0) {
    printf("# Done .... \n");
    printf("# Charm++ + Kokkos MiniMD output ...\n");
    printf("# Run Settings: \n");
    printf("\t# Chares: %i\n", num_chares);
    printf("\t# Host Threads: %i\n", Kokkos::DefaultHostExecutionSpace().concurrency());
    printf("\t# Inputfile: %s\n", input_file.c_str());
    printf("\t# Datafile: %s\n", in_datafile.empty() ? "None" : in_datafile.c_str());
    printf("# Physics Settings: \n");
    printf("\t# ForceStyle: %s\n", in_forcetype == FORCELJ ? "LJ" : "EAM");
    printf("\t# Force Parameters: %2.2lf %2.2lf\n",in_epsilon,in_sigma);
    printf("\t# Units: %s\n", in_units == 0 ? "LJ" : "METAL");
    printf("\t# Atoms: %i\n", atom.natoms);
    printf("\t# Atom types: %i\n", atom.ntypes);
    printf("\t# System size: %2.2lf %2.2lf %2.2lf (unit cells: %i %i %i)\n", atom.box.xprd, atom.box.yprd, atom.box.zprd, in_nx, in_ny, in_nz);
    printf("\t# Density: %lf\n", in_rho);
    printf("\t# Force cutoff: %lf\n", force->cutforce);
    printf("\t# Timestep size: %lf\n", integrate.dt);
    printf("# Technical Settings: \n");
    printf("\t# Neigh cutoff: %lf\n", neighbor.cutneigh);
    printf("\t# Half neighborlists: %i\n", neighbor.halfneigh);
    printf("\t# Team neighborlist construction: %i\n", neighbor.team_neigh_build);
    printf("\t# Neighbor bins: %i %i %i\n", neighbor.nbinx, neighbor.nbiny, neighbor.nbinz);
    printf("\t# Neighbor frequency: %i\n", neighbor.every);
    printf("\t# Sorting frequency: %i\n", integrate.sort_every);
    printf("\t# Thermo frequency: %i\n", thermo.nstat);
    printf("\t# Ghost Newton: %i\n", ghost_newton);
    printf("\t# Use intrinsics: %i\n", force->use_sse);
    printf("\t# Do safe exchange: %i\n", comm->do_safeexchange);
