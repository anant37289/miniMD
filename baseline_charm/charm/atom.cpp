...
void Atom::pack_comm(int n, int_1d_view_type list_in, float_1d_view_type buf_in, int* pbc_flags_in)
{
  ...
  Kokkos::DefaultExecutionSpace instance = pack_instance;
...
void Atom::unpack_comm(int n, int first_in, float_1d_view_type buf_in)
{
  ...
  Kokkos::DefaultExecutionSpace instance = unpack_instance;
...
void Atom::sort(Neighbor &neighbor)
{
  ...
  if(copy_size<nmax) {
    ...
    resize_unmanaged_2d_views(x_copy, nmax, x_copy.extent(1), kokkos_instance_stream(compute_instance));
    resize_unmanaged_2d_views(v_copy, nmax, x_copy.extent(1), kokkos_instance_stream(compute_instance));
    resize_unmanaged_1d_views(type_copy, nmax, kokkos_instance_stream(compute_instance));
    copy_size = nmax;
  }
